...
/*
class CreateReserve extends StatefulWidget {
  const CreateReserve({super.key});

  @override
  State<CreateReserve> createState() => _CreateReserveState();
}

class _CreateReserveState extends State<CreateReserve> {
  final _formKey = GlobalKey<FormState>();
  final TextEditingController _huespedController = TextEditingController();
  final TextEditingController _alojamientoController = TextEditingController();
  final TextEditingController _fechaCheckInController = TextEditingController();
  final TextEditingController _fechaCheckOutController = TextEditingController();
  final TextEditingController _numeroHuespedesController = TextEditingController();
    ...
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        if (response.statusCode == 201) {
          ...
          // Si todo bien
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Se ha reservado con éxito')),
          );
          formProvider.reset();
