...
class _ExploreScreenState extends State<ExploreScreen> with SingleTickerProviderStateMixin {
  ...
  List<String> tiposUnicos() {
    final tipos = _todosLosAlojamientos
        .map((a) => a.tipoAlojamiento)
        .whereType<String>()  // Filtra null y valores no String
        .toSet()
        .toList();
    tipos.sort();
    return tipos;
          ...
          Expanded(
            child: FutureBuilder<List<Alojamiento>>(
              ...
              builder: (context, snapshot) {
                ...
                // Guardamos los datos en las listas si aún no están seteadas
                if (_todosLosAlojamientos.isEmpty) {
                  _todosLosAlojamientos = snapshot.data!;
                  _alojamientosFiltrados = snapshot.data!;
...
// Aquí tu clase CardAccomodations, igual que la tienes definida
...
child: Column(
  ...
  children: [
    ...
    Text(
      ...
      style: const TextStyle(color: Colors.black87), // más visible
    ),
    const SizedBox(height: 6),
    Row(
      children: [
