import 'package:desole_app/features/guest/explore/widgets/detail_screen.dart';
...
class _CardAccomodationsState extends State<CardAccomodations> {
  ...
  Future<void> _loadFirstImage() async {
    try {
      final fotos = await _service.getPhotosAccomadations(widget.destino.id);
      if (fotos.isNotEmpty) {
  ...
  Widget build(BuildContext context) {
    return InkWell(
      onTap: () {
        Navigator.push(
          context,
          MaterialPageRoute(builder: (context) => DetailScreen(destino: widget.destino)),
        );
      },
      child: Card(
